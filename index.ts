...
$(() => {
  ...
  const dataInitialization: any = async () => {
    utility.allXchangeData = await chromeStoreApi.getDataFromcoinplace("GetAllKoin");
    const favkoins = await chromeStoreApi.getDataFromStore("coinplacefav");
  ...
  function chooseFavs() {
    $(".card-header").on("click", async (event) => {
      ...
      chromeStoreApi.saveDateToStore({ coinplacefav: utility.favKoins });
    });
  }
